...
account is funding-blocked â€” substitute IBKR auth for T5 until the Tradovate
...
n=63 trades over 4 weeks; bootstrap CI95 spans zero (âˆ’0.087 to +0.512). We do
not know yet whether the edge is real â€” live sample closes that gap. Every
decision below exists to protect the sample.

---

## 2. Trigger rules â€” WHAT MUST BE TRUE BEFORE I CLICK START
...
| T3 | Paper baseline current | `ls -lt var/eta_engine/state/paper_run/paper_run_report.json` | â‰¤ 7 days old |
...
| T6 | Market session | **RTH ONLY** 09:30â€“16:00 ET (see Â§13) | inside window |
...
If ANY is RED â†’ DO NOT START. Fix or abort the session.

---

## 3. Block rules â€” WHEN TO HALT IN-SESSION (automated + manual)

### Automated kill-switch (runs continuously)
Trips `FLATTEN_ALL + CRITICAL` when any of:
- Realized portfolio DD â‰¥ 50% of session cap
...
- Live-vs-paper drift **RED** on any run of:
  `python -m eta_engine.scripts.diamond_live_paper_drift --json`
  when `severity_counts.BAD > 0`
- MNQ latency scorecard **RED** on any run of:
  `python -m eta_engine.scripts.mnq_latency_scorecard --hours 24 --json`
...
## 4. Size rules â€” WHAT QTY IS ALLOWED
...
**Phase 5 (weeks 2â€“4):** no change until n â‰¥ 60 live trades AND drift tracker
is GREEN 3 days running.

**Phase 6 (after n â‰¥ 120):** only advance to 2 contracts if:
- Live walk-forward STABLE (pass_rate â‰¥ 67% of folds at +0.05R gate)
- Live bootstrap CI95 EXCLUDES zero
- Zero RED drift days in the past 2 weeks

**NEVER** add a second bot in Phase 5â€“6 without running
`portfolio_correlation_audit.py` against the candidate. If daily-R rho > 0.4,
the candidate does not qualify as diversification â€” it qualifies as
duplicate risk.

---

## 5. Stop rules â€” HOW TO SHUT DOWN

### Graceful (preferred)
```bash
# in the live terminal:
Ctrl-C  â†’  wait for "runtime_stop" alert in logs/eta_engine/alerts_log.jsonl
```
...
1. Open the **active broker's** UI:
   ...
   - Tradovate is DORMANT â€” see `live_launch_runbook.md` Appendix A if it has un-dormanted.
2. Orders tab â†’ **Cancel All Working**.
3. Positions tab â†’ **Flatten All** (market).
4. Immediately: `python eta_engine/scripts/_trade_journal_reconcile.py --hours 1`.
5. Snapshot `logs/eta_engine/alerts_log.jsonl` â†’ `docs/incidents/YYYYMMDD.jsonl`.
6. Append root-cause entry to `var/eta_engine/state/kill_log.json`.

---

## 6. Failure definition â€” WHAT COUNTS AS A BAD SESSION

A session is a **failure** and must be reviewed before the next one if ANY:
- Realized session PnL < âˆ’$200 (4Ã— 1R stop, unusual for a single session).
...
The reconcile and kill-switch commands should exit 0. The MNQ latency
scorecard exits 0 = GREEN, 1 = YELLOW, 2 = RED based on structured
`entry_fill_age_s` telemetry from canonical MNQ closes, using broker
fill time when the venue exposes it and otherwise falling back to
router sidecar/adoption timing, with confirmed `FILL_AGE_EXCEEDED`
alerts as a hard corroborating backstop. The drift
receipt is advisory: HALT next session if `severity_counts.BAD > 0`.
...
(default 20) drops below `expectancy_threshold_r` (default âˆ’0.10R), provided
...
| min_trades_for_check   | 10      | Warm-up â€” no check below this trade count         |
| expectancy_threshold_r | -0.10   | Trip if rolling exp_R â‰¤ this                      |

This catches the regime-shift scenario the v2.2 NQ transfer test exposed
(`v2_2_nq_transfer.md` â€” Jan-Feb 2026 NQ data showed v2.2 bleeding âˆ’0.20R
per trade for â‰¥5 weeks). Daily-DD-20% global kill alone would not have
...
1. n_trades â‰¥ 120
2. walk-forward pass_rate â‰¥ 67%  (STABLE verdict)
...
6. realized expectancy â‰¥ +0.10R (half of paper figure â€” conservative buffer)

Missing any â†’ STAY at 1 contract. Re-evaluate in 1 week.
...
- Weekend sessions while in Phase 4â€“5.
...
|---|---|:-:|
| **trend_bias** | sign of EMA9 âˆ’ EMA21 on 5-min bars | 40.8% of score |
| **vol_regime** | ATR(14) in sweet-spot range 0.3â€“0.8Ã— average | 25.4% of score |
| **ADX filter** (overlay) | ADX(14) â‰¥ 18 required to enter (chop suppression) | pass/fail gate |

`funding_skew` is 0 every bar (futures has no funding). `onchain_delta` and `sentiment`
are crypto-tuned and contribute noise from neutral placeholders â€” they do not materially
move the MNQ trigger.

**Operator rule of thumb:** MNQ trades when the 5-min EMA9 separates from EMA21 by enough,
in a normal-ATR regime, and the market is trending (ADX â‰¥ 18). If you want to know why a
trade fired, inspect those three numbers â€” they reconstruct â‰¥95% of the confluence score.

The scorer's max observed value on MNQ is 6.42; the 20Ã— and 75Ã— leverage rungs at 7.0 and
...
## 12. Execution-latency rules â€” fill-age gate + RTH-only
...
|---             :|---                     |---                            |
| â‰¤ 1             | 0R (baseline)          | Trade proceeds normally       |
| 2               | +0.036R                | Log as **warning**, continue  |
| 3               | **âˆ’0.089R**            | Cancel pending, log `FILL_AGE_EXCEEDED` |
| 5+              | âˆ’0.053R                | Cancel pending + halt session |
...
- `mnq_latency_scorecard.py` reports GREEN when no confirmed
  `FILL_AGE_EXCEEDED` alerts fire in the window, YELLOW when one fires,
  and RED when two or more fire. It also summarizes realized PnL, avg R,
  and win rate from canonical live MNQ closes for the same window.
...
## 13. Session-hour rule â€” RTH ONLY until further notice
...
| **US evening**     | 21 | **âˆ’0.167R**|33.3% |
| Asia open (23-03)  |  6 | âˆ’0.167R    |33.3% |
| London (03-09)     |  6 | +0.250R    |50.0% |

**Rule:** Live sessions are restricted to **09:30â€“16:00 ET** (RTH). Any trade
...
over nâ‰¥40 trades, we reopen US-evening on a fresh paper sample with slippage
model calibrated for evening volume. Decision framework: `mnq_session_hour_stress.md`.

## 14. ATR-floor rule â€” v2.1 (superseded by v2.2, NOT live)
...
actually lives. Implementation in `make_ctx_builder_real` â€” sets
...
|---           |---:|---:|
| trades       | 63 | 37 |
| expectancy   | +0.111R | **+0.358R** (3.2Ã—) |
| win rate     | 49.2% | 59.5% |
| total R      | +6.99 | +13.26 |
| max DD       | 2.47% | 2.18% |

**Walk-forward:** 4-week folds Ã— 1-week stride on the full 6.6-week cache:
...
- If daily trade count is suspiciously low, check that ATR history is
  populating â€” the filter needs 30 ATR samples before it activates
  (`len(atr_history_cache) >= 30`).

**Re-validation gate:** the live-promotion decision is attached to v2.2 Â§15,
...
`docs/atr_floor_validation_v2_2.md` for the 2Ã—2 corner table.

Details: `docs/atr_floor_validation.md` (original v2.1 study) +
`docs/atr_floor_validation_v2_2.md` (v2.2-stack re-sweep + 2Ã—2 ablation).

## 15. Thursday blacklist â€” v2.2 paper config (NOT live)
...
`make_ctx_builder_real`'s `_wrap_dow` â€” outermost wrapper, short-circuits
...
**Walk-forward:** 4-week folds Ã— 1-week stride, n_folds=3:
...
**All three folds improved.** Two folds flipped sign (-0.008â†’+0.093,
-0.018â†’+0.122). Third fold strengthened (+0.179â†’+0.252). No fold worsened.
...
- v2.2 stays in **paper only**. 3 folds of walk-forward is too thin even at
  100% pass to trip the live gate (which requires â‰¥6 folds).
...
- **Do NOT further tighten threshold on top of v2.2.** The v2.3 candidate
  (threshold=5.5 on v2.2) projects to n=10, -0.10R â€” score-bucket
  non-monotonicity means threshold and Thu-blacklist are overlapping
  filters. See `docs/v2_2_dow_filter_validation.md` Â§Compound candidate
  v2.3 killed.

**Re-validation gate:** once the TradingView cache reaches â‰¥13 weeks of
MNQ 5-min bars, re-run `dow_filter_validation.py`. Promote to live only
if v2.2 pass_rate â‰¥ 67% across â‰¥6 folds.

Details: `docs/v2_2_dow_filter_validation.md`.

## Lineage

- `docs/canonical_v1_verdict_full.md` â€” why NQ/crypto were killed
- `docs/mnq_feature_contribution_audit.md` â€” 20/80 axis analysis (+ vol_regime miscalibration flag)
- `docs/mnq_session_hour_stress.md` â€” session-hour stress analysis (RTH-only rule)
- `docs/mnq_latency_volatility_interaction.md` â€” panic-ATR edge concentration
- `docs/atr_floor_validation.md` â€” v2.1 ATR-floor filter validation + ship decision
- `docs/atr_floor_validation_v2_2.md` â€” v2.2-stack re-sweep + 2Ã—2 corner ablation (2026-04-24)
- `docs/v2_0_vs_v2_1_comparison.md` â€” v2.0 vs v2.1 dimension-by-dimension comparison
- `docs/v2_1_trade_distribution.md` â€” v2.1 trade-log distribution analysis (Thu blacklist motivation)
- `docs/v2_2_dow_filter_validation.md` â€” v2.2 Thursday blacklist validation + ship decision
- `docs/v2_2_dow_placebo.json` â€” DOW placebo sweep (2026-04-24); Mon-blacklist comparable to Thu
- `docs/v2_2_nq_transfer.md` â€” cross-symbol regime-shift finding (lesson #14)
- `docs/v2_2_promotion_attack.md` â€” adversarial agent review (risk-advocate + quant-researcher â†’ HOLD)
- `docs/v2_2_block_bootstrap.md` â€” block-bootstrap CI on v2.2 (gate #3 fail evidence)
- `docs/v2_2_walk_forward_ci_corrected.md` â€” overlap-corrected fold-mean CI (gate #2 fail evidence)
- `docs/v2_2_dsr.md` â€” DSR computation (gate #4 fail by ~12Ã—)
- `docs/v2_2_threshold_wf_sensitivity.md` â€” gate #8 PASS for confluence_threshold
- `docs/v2_2_atr_stop_mult_wf_sensitivity.json` â€” gate #8 PASS for atr_stop_mult
- `docs/v2_2_target_r_multiple_wf_sensitivity.json` â€” gate #8 PASS for target_r_multiple (tied)
- `docs/v2_2_max_trades_per_day_wf_sensitivity.json` â€” gate #8 PASS for max_trades_per_day
- `docs/v2_2_gate_8_knob_summary.md` â€” aggregate gate #8 PASS (4/4 knobs at WF argmax)
- `docs/trial_log.json` â€” cumulative N_trials log for DSR (gate #4)
- `docs/overrides_p9_real_mnq_only_v2.json` â€” v2.0 (trending_only, no floor)
- `docs/overrides_p9_real_mnq_only_v2_1.json` â€” v2.1 intermediate (floor=1.0, no DOW)
- `docs/overrides_p9_real_mnq_only_v2_2_dow_thu.json` â€” **v2.2 paper config** (floor=1.0 + Thu blacklist, not live)
- `docs/live_launch_runbook.md` â€” step-by-step phase 1-6 runbook
- `var/eta_engine/state/kill_log.json` â†’ entries `ETA_PORTFOLIO_COMBINED_v1` + `MNQ_V2_FAMILY_CACHE_CONTAGION`
- `eta_engine/scripts/diamond_live_paper_drift.py` â€” daily live-vs-paper drift receipt
- `eta_engine/scripts/mnq_latency_scorecard.py` â€” daily confirmed fill-age scorecard
- `eta_engine/scripts/slippage_stress_mnq.py` â€” cost-robustness check
- `eta_engine/scripts/overlay_ablation_mnq.py` â€” overlay walk-forward validator
- `eta_engine/scripts/live_tiny_preflight_dryrun.py` â€” T1â€“T8 gates
- `eta_engine/scripts/block_bootstrap.py` â€” gate #3 implementation (moving-block bootstrap)
- `eta_engine/scripts/walk_forward_ci.py` â€” gate #2 implementation (overlap-corrected CI)
- `eta_engine/scripts/trial_counter.py` â€” gate #4 implementation (cumulative N_trials + DSR)
- `eta_engine/scripts/v2_2_knob_wf_sensitivity.py` â€” gate #8 implementation (knob WF sweep)
- `eta_engine/scripts/v2_2_trade_dump.py` â€” full-sample trade dumper (feeds bootstrap/DSR)
- `eta_engine/scripts/v2_2_dow_placebo.py` â€” DOW methodology placebo
- `eta_engine/scripts/v2_2_nq_transfer.py` â€” cross-symbol transfer harness
- `eta_engine/configs/kill_switch.yaml` â€” automated halt config (now with `slow_bleed:` block)
- `eta_engine/core/kill_switch_runtime.py` â€” slow-bleed circuit breaker (lesson #14, #19)
- `eta_engine/docs/macro_calendar.json` â€” news_blacklist filter calendar (lesson #22)
