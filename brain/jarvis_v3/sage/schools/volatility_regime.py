...
class VolatilityRegimeSchool(SchoolBase):
    ...
    def analyze(self, ctx: MarketContext) -> SchoolVerdict:
        ...
        if vol_ratio >= 1.5:
            ...
            rationale = f"vol expanding sharply (ratio={vol_ratio:.2f}) -- WIDER stops + smaller size warranted"
            regime = "expanding"
        elif vol_ratio <= 0.6:
            conv = min(0.75, 0.30 + (0.6 - vol_ratio) * 0.5)
            rationale = f"vol contracting (ratio={vol_ratio:.2f}) -- squeeze setup; breakout pending"
            regime = "contracting"
        else:
            conv = 0.20
