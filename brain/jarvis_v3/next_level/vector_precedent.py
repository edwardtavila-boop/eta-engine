...
class PrecedentVectorEntry(BaseModel):
    ...
    vector:          list[float] = Field(min_length=EMBEDDING_DIM,
...
class VectorPrecedentStore:
    ...
    def record(
        ...
        *,
        ...
        vec = _embed(
            regime=regime, session_phase=session_phase,
        ...
        entry = PrecedentVectorEntry(
            id=entry_id, ts=ts, regime=regime,
            session_phase=session_phase, event_category=event_category,
            binding_constraint=binding_constraint, tags=tags or [],
            action=action, outcome_correct=outcome_correct,
            realized_r=realized_r, vector=vec, note=note,
        )
    ...
    def search(
        ...
        *,
        ...
        event_category: str = "", binding_constraint: str = "",
        tags: list[str] | None = None,
        numeric_features: dict[str, float] | None = None,
        k: int = 5,
    ) -> list[NeighborResult]:
    ...
    def synthesize(
        ...
        hit_rate = (sum(n.entry.outcome_correct for n in labeled
                        if n.entry.outcome_correct) / len(labeled)
        ...
        if mean_r is not None and mean_r > 0.3 and (hit_rate or 0) >= 0.55:
            sugg = (
                f"semantic precedent favors TRADE "
                f"(sim={sim_mean:.2f}, mean_r={mean_r:+.2f})"
            )
        elif mean_r is not None and mean_r < -0.3:
            sugg = (
                f"semantic precedent unfavorable "
                f"(sim={sim_mean:.2f}, mean_r={mean_r:+.2f}) -- stand aside"
            )
        else:
...
def _embed(
    *,
    regime: str, session_phase: str,
    event_category: str, binding_constraint: str,
    tags: list[str], numeric_features: dict[str, float],
) -> list[float]:
