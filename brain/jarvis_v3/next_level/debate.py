...
def skeptic_argue(
    *,
    ...
    if regime_confidence < 0.5:
        reasons.append(
            f"regime {regime} is only {regime_confidence:.0%} confident -- classifier may be wrong",
        )
    if events_count > 2:
        reasons.append(
...
def historian_argue(
    *,
    ...
    if precedent_mean_r is not None and precedent_mean_r > 0.3 and (precedent_win_rate or 0) >= 0.5:
        vote = "APPROVE"
...
def hold_debate(
    ...
    summary = (
        f"{winner[0]} wins with score {winner[1]:.2f} (margin {margin:.2f}, consensus={'yes' if consensus else 'no'})"
    )
    return DebateVerdict(
        ts=now,
        final_vote=winner[0],
