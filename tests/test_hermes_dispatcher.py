...
def test_dispatch_routes_feed_degraded_to_warn() -> None:
    ...
    rec = {
        ...
        "bot_id": "",
        "class": "MNQ",
        "details": {"feed": "yfinance", "symbol": "MNQ", "ok": 3, "empty": 7, "empty_rate": 0.7, "threshold": 0.3},
        "severity": "WARN",
    }
    with (
        patch(
            "eta_engine.brain.jarvis_v3.hermes_bridge.send_alert",
            fake_send,
        ),
        patch(
            "threading.Thread",
...
def test_dispatch_class_loss_freeze_is_critical() -> None:
    ...
    with patch("threading.Thread", side_effect=_capture), patch("asyncio.run", side_effect=_capture_run):
        rec = {
            "layer": "v25",
            "event": "class_loss_freeze",
            "bot_id": "btc_a",
