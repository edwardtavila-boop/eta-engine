...
def _write_sig_fill_pair(
    tmp_path: Path, n: int = 30, latency_seconds: float = 1.0, strategy_id: str = "book_imbalance_v1"
) -> None:
...
def test_fill_latency_fail_when_too_slow(tmp_path: Path) -> None:
    ...
    _write_sig_fill_pair(tmp_path, n=30, latency_seconds=15.0, strategy_id="microprice_drift_v1")
    report = fl.run_latency_audit(
...
def test_fill_latency_decay_thresholds_per_strategy() -> None:
    ...
    assert fl.DECAY_THRESHOLDS["microprice_drift_v1"] < fl.DECAY_THRESHOLDS["book_imbalance_v1"]
    assert fl.DECAY_THRESHOLDS["aggressor_flow_v1"] > fl.DECAY_THRESHOLDS["book_imbalance_v1"]


def test_fill_latency_uses_default_threshold_for_unknown_strategy(tmp_path: Path) -> None:
    _write_sig_fill_pair(tmp_path, n=20, latency_seconds=1.0, strategy_id="unknown_v1")
    report = fl.run_latency_audit(
...
def test_fill_latency_multi_symbol_specs_in_strategy() -> None:
    ...
    for sym in ("MNQ", "NQ", "ES", "M2K", "MYM", "GC", "SIL", "CL", "NG", "ZN", "ZB", "MBT", "BTC", "M6E"):
