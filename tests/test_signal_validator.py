...
def test_long_with_stop_below_entry_passes():
    r = validate_signal(side="LONG", entry=100.0, stop=99.0, target=102.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert r.ok, r.failures


def test_long_with_stop_above_entry_fails():
    r = validate_signal(side="LONG", entry=100.0, stop=101.0, target=102.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
...
def test_short_with_stop_below_entry_fails():
    r = validate_signal(side="SHORT", entry=100.0, stop=99.0, target=98.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
    codes = {f.code for f in r.failures}
    assert "stop_side_inverted" in codes
...
def test_short_with_stop_above_entry_passes():
    r = validate_signal(side="SHORT", entry=100.0, stop=101.0, target=98.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert r.ok, r.failures


def test_long_with_target_below_entry_fails():
    r = validate_signal(side="LONG", entry=100.0, stop=99.0, target=98.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
    codes = {f.code for f in r.failures}
...
def test_short_with_target_above_entry_fails():
    r = validate_signal(side="SHORT", entry=100.0, stop=101.0, target=102.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
    codes = {f.code for f in r.failures}
    assert "target_side_inverted" in codes
...
def test_invalid_side_string_fails():
    r = validate_signal(side="WTF", entry=100.0, stop=99.0, target=102.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
    codes = {f.code for f in r.failures}
...
def test_rr_below_floor_rejected():
    ...
    r = validate_signal(side="LONG", entry=100.0, stop=99.0, target=100.05, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
...
def test_rr_above_ceiling_rejected():
    ...
    r = validate_signal(side="LONG", entry=100.0, stop=99.99, target=200.0, qty=1.0, equity=10000000.0, point_value=2.0)
    assert not r.ok
...
def test_stop_more_than_20_pct_from_entry_rejected():
    ...
    r = validate_signal(side="LONG", entry=100.0, stop=70.0, target=200.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert not r.ok
...
def test_stop_within_20_pct_passes():
    ...
    r = validate_signal(side="LONG", entry=100.0, stop=95.0, target=110.0, qty=1.0, equity=10000.0, point_value=2.0)
    assert r.ok
...
def test_zero_qty_rejected():
    r = validate_signal(side="LONG", entry=100.0, stop=99.0, target=102.0, qty=0.0, equity=10000.0, point_value=2.0)
    assert not r.ok
...
def test_huge_notional_rejected():
    ...
    r = validate_signal(
        side="LONG", entry=20000.0, stop=19999.0, target=20002.0, qty=10000.0, equity=10000.0, point_value=2.0
    )
    assert not r.ok
    codes = {f.code for f in r.failures}
...
def test_normal_futures_notional_passes():
    ...
    r = validate_signal(
        side="LONG", entry=20000.0, stop=19990.0, target=20020.0, qty=5.0, equity=50000.0, point_value=2.0
    )
    assert r.ok, r.failures
...
def test_typical_retail_futures_size_passes():
    ...
    r_over = validate_signal(
        side="LONG", entry=27000.0, stop=26990.0, target=27020.0, qty=10.0, equity=10000.0, point_value=2.0
    )
    r_under = validate_signal(
        side="LONG", entry=27000.0, stop=26990.0, target=27020.0, qty=9.0, equity=10000.0, point_value=2.0
    )
...
def test_open_rejects_long_stop_above_entry():
    ...
    with pytest.raises(ValueError, match="LONG stop"):
        _Open(
            ...
            entry_price=100.0,
            stop=101.0,
            target=105.0,  # stop ABOVE entry — BAD
            risk_usd=100.0,
            confluence=5.0,
            leverage=1.0,
        )
...
def test_open_rejects_short_stop_below_entry():
    ...
    with pytest.raises(ValueError, match="SHORT stop"):
        _Open(
            ...
            entry_price=100.0,
            stop=99.0,
            target=95.0,  # stop BELOW entry — BAD
            risk_usd=100.0,
            confluence=5.0,
            leverage=1.0,
        )
...
def test_open_rejects_long_target_below_entry():
    ...
    with pytest.raises(ValueError, match="LONG target"):
        _Open(
            ...
            entry_price=100.0,
            stop=99.0,
            target=95.0,  # target BELOW entry — BAD
            risk_usd=100.0,
            confluence=5.0,
            leverage=1.0,
        )
