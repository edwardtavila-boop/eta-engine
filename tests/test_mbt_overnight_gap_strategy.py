...
def test_gap_up_at_rth_open_fires_long() -> None:
    ...
    open_bar = _bar(
        next_day_open,
        high=60_650.0, low=60_450.0,
        open_=60_500.0, close=60_600.0, volume=1000.0,
    )
    ...
    if out is None:
        ...
        next_bar = _bar(
            next_ts, high=60_750.0, low=60_550.0,
            open_=60_600.0, close=60_700.0, volume=1000.0,
        )
        out = s.maybe_enter(next_bar, hist, 10_000.0, bcfg)
    assert out is not None, (
        f"expected LONG to fire on gap-up continuation; stats={s.stats}"
    )
    assert out.side == "BUY"
...
def test_gap_down_fires_short_continuation() -> None:
    ...
    for i in range(5):
        ts = prior_day + timedelta(minutes=i * 5)
        bar = _bar(
            ts, high=60_050.0, low=59_950.0, close=60_000.0,
            volume=1000.0,
        )
        s.maybe_enter(bar, hist, 10_000.0, bcfg)
        hist.append(bar)
    ...
    open_bar = _bar(
        next_day_open,
        high=59_550.0, low=59_300.0,
        open_=59_500.0, close=59_400.0, volume=1000.0,
    )
    out = s.maybe_enter(open_bar, hist, 10_000.0, bcfg)
    if out is None:
        hist.append(open_bar)
        next_ts = next_day_open + timedelta(minutes=5)
        next_bar = _bar(
            next_ts, high=59_450.0, low=59_200.0,
            open_=59_400.0, close=59_250.0, volume=1000.0,
        )
        out = s.maybe_enter(next_bar, hist, 10_000.0, bcfg)
    assert out is not None, (
        f"expected SHORT continuation on gap-down; stats={s.stats}"
    )
...
def test_small_gap_below_one_atr_is_rejected() -> None:
    ...
    cfg = _permissive_cfg(
        atr_period=5, warmup_bars=4,
        min_gap_atr_mult=1.0, max_gap_atr_mult=2.0,
        min_session_gap_hours=2.0,
    )
    s = MBTOvernightGapStrategy(cfg)
    bcfg = _config()
    ...
    open_bar = _bar(
        next_day_open,
        high=60_100.0, low=60_020.0,
        open_=60_050.0, close=60_080.0, volume=1000.0,
    )
    out = s.maybe_enter(open_bar, hist, 10_000.0, bcfg)
    assert out is None
    assert s._n_gaps_too_small >= 1
...
def test_weekend_gap_monday_after_friday_close() -> None:
    ...
    open_bar = _bar(
        mon_open,
        high=60_700.0, low=60_450.0,
        open_=60_500.0, close=60_650.0, volume=1000.0,
    )
    out = s.maybe_enter(open_bar, hist, 10_000.0, bcfg)
    if out is None:
        hist.append(open_bar)
        ...
        next_bar = _bar(
            next_ts, high=60_750.0, low=60_600.0,
            open_=60_650.0, close=60_720.0, volume=1000.0,
        )
        out = s.maybe_enter(next_bar, hist, 10_000.0, bcfg)
    assert out is not None, (
        f"expected weekend-gap LONG continuation; stats={s.stats}"
    )
    assert out.side == "BUY"
...
def test_red_bar_after_gap_up_does_not_fire() -> None:
    ...
    cfg = _permissive_cfg(
        atr_period=5, warmup_bars=4,
        min_gap_atr_mult=1.0, max_gap_atr_mult=10.0,
        min_session_gap_hours=2.0,
    ...
    assert out is None, (
        f"continuation should reject red-bar after gap-up; "
        f"stats={s.stats}"
    )
