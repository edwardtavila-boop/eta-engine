...
def _write_depth_snapshots(d: Path, symbol: str, dt: datetime, snapshots: list[dict]) -> None:
...
def test_l2_overlay_sweep_real_stop_run(depth_dir: Path) -> None:
        ...
        [
            {
                ...
                "epoch_s": snap_epoch,
                "symbol": "MNQ",
                "bids": [{"price": 100.0, "size": 30}, {"price": 99.75, "size": 25}, {"price": 99.5, "size": 20}],
    ...
    r = l2_overlay.confirm_sweep_with_l2(symbol="MNQ", swept_level=100.0, touch_dt=dt, side="LONG", min_stop_qty=50)
    assert r.passed is True
...
def test_l2_overlay_poc_pull_confirmed(depth_dir: Path) -> None:
    ...
    r = l2_overlay.confirm_poc_pull_with_l2(symbol="MNQ", entry_dt=dt, entry_side="LONG", min_imbalance_ratio=2.0)
    assert r.passed is True
...
def test_l2_overlay_anchor_had_liquidity(depth_dir: Path) -> None:
    ...
    r = l2_overlay.confirm_anchor_touch_with_l2(
        symbol="MNQ", anchor_price=101.0, touch_dt=dt, min_qty_within_pts=5.0, min_qty=30
    )
    assert r.passed is True
...
def _snap(bid_qtys: list[int], ask_qtys: list[int], mid: float = 100.0, spread: float = 0.25) -> dict:
    return {
...
def test_evaluate_snapshot_respects_max_trades_per_day() -> None:
    cfg = bis.BookImbalanceConfig(entry_threshold=1.5, consecutive_snaps=1, max_trades_per_day=2)
    state = bis.BookImbalanceState()
    for _ in range(5):
...
def test_l2_backtest_no_data_returns_zero_signals(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_l2_backtest_synthetic_signal_to_trade(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
