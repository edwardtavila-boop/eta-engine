...
def _make_strong_book_imbalance_snaps(n: int = 50) -> list[dict]:
    ...
    for i in range(n):
        ...
        snaps.append({
            ...
            "symbol": "MNQ",
            "bids": [{"price": 99.75 - j * 0.25, "size": 50 - j * 5}
                      for j in range(3)],
            "asks": [{"price": 100.25 + j * 0.25, "size": 10 - j * 2}
                      for j in range(3)],
            "spread": 0.5,
            "mid": 100.0,
        })
...
def test_multistrat_all_four_fire_on_bullish_regime() -> None:
    ...
    bis_cfg = bis.BookImbalanceConfig(
        ...
        spread_min_ticks=1, spread_max_ticks=10,
        max_trades_per_day=10, min_stop_ticks=1,
    )
    bis_signals = []
    for s in snaps:
        sig = bis.evaluate_snapshot(s, bis_cfg, bis_state, atr=2.0,
                                       symbol="MNQ")
        if sig is not None:
    ...
    ag_cfg = ag.AggressorFlowConfig(window_bars=5, consecutive_bars=2,
                                       entry_threshold=0.3,
    ...
    mp_cfg = mp.MicropriceConfig(drift_threshold_ticks=0.5,
                                    consecutive_snaps=2,
                                    max_trades_per_day=10)
    mp_signals = []
    for s in snaps:
        sig = mp.evaluate_snapshot(s, mp_cfg, mp_state, atr=2.0,
                                     symbol="MNQ")
        if sig is not None:
    ...
    fp_cfg = fp.FootprintAbsorptionConfig(prints_size_z_min=0.5,
    ...
    for i, sz in enumerate([3, 4, 5, 6, 4, 5, 7, 3, 5, 6, 4, 5, 6, 5, 4]):
        fp_state.recent_prints.append({
            ...
            "price": 100.0, "size": sz, "side": "SELL",
            "mid_before": 100.0, "mid_after": 100.0,
            "opposite_qty_before": 50, "opposite_qty_after": 49,
        })
    fp_state.recent_prints.append({
        "ts": base + timedelta(seconds=20),
        "price": 100.0, "size": 80, "side": "SELL",
        "mid_before": 100.0, "mid_after": 100.0,
        "opposite_qty_before": 50, "opposite_qty_after": 49,
    })
...
def test_multistrat_portfolio_limits_blocks_third_stacked_long(
        tmp_path: Path) -> None:
    ...
    decision = plim.check_portfolio_limits(
        symbol="MNQ", side="LONG", qty=1, _fill_path=fills,
        _log_path=tmp_path / "plim.jsonl")
    assert decision.blocked is True
    assert "same_side_stacking" in decision.reason


def test_multistrat_portfolio_limits_allows_offsetting_short(
        tmp_path: Path) -> None:
    ...
    decision = plim.check_portfolio_limits(
        symbol="MNQ", side="SHORT", qty=1, _fill_path=fills,
        _log_path=tmp_path / "plim.jsonl")
...
def test_news_blackout_does_not_block_unaffected_symbol(
        tmp_path: Path) -> None:
    ...
    events.write_text(json.dumps({
        "start": nfp.start, "end": nfp.end,
        "reason": nfp.reason, "symbols": nfp.symbols,
        "note": nfp.note,
    }) + "\n", encoding="utf-8")
    when = datetime.fromisoformat(nfp.start.replace("Z", "+00:00")) \
        + timedelta(minutes=5)
...
def test_fuse_blows_after_threshold_losses(tmp_path: Path) -> None:
    ...
    for _ in range(5):
        fuse.record_outcome(
            ...
            fuse_threshold=5, _path=fuse_path,
        )
    state = fuse.check_fuse("book_imbalance_v1", "MNQ",
                              cooldown_seconds=3600, _path=fuse_path)
...
def test_fuse_winning_trade_resets_counter(tmp_path: Path) -> None:
    ...
    for _ in range(4):
        fuse.record_outcome(
            "microprice_drift_v1", "MNQ", won=False, _path=fuse_path)
    state = fuse.record_outcome(
        "microprice_drift_v1", "MNQ", won=True, _path=fuse_path)
...
def test_fuse_each_strategy_tracked_independently(tmp_path: Path) -> None:
    ...
    for _ in range(5):
        fuse.record_outcome("book_imbalance_v1", "MNQ", won=False,
                              _path=fuse_path)
    # Footprint still OK
    state_bis = fuse.check_fuse("book_imbalance_v1", "MNQ",
                                  _path=fuse_path)
    state_fp = fuse.check_fuse("footprint_absorption_v1", "MNQ",
                                 _path=fuse_path)
...
def test_fuse_per_symbol_isolation(tmp_path: Path) -> None:
    ...
    for _ in range(5):
        fuse.record_outcome("book_imbalance_v1", "MNQ", won=False,
                              _path=fuse_path)
    state_mnq = fuse.check_fuse("book_imbalance_v1", "MNQ",
                                  _path=fuse_path)
    state_es = fuse.check_fuse("book_imbalance_v1", "ES",
                                 _path=fuse_path)
...
def test_fuse_cooldown_auto_resets(tmp_path: Path) -> None:
    ...
    for _ in range(5):
        fuse.record_outcome(
            "book_imbalance_v1", "MNQ", won=False, ts=past,
            _path=fuse_path,
        )
...
def test_walk_forward_each_strategy_produces_signals_on_synthetic() -> None:
    ...
    bis_cfg = bis.BookImbalanceConfig(
        ...
        spread_min_ticks=1, spread_max_ticks=10,
        max_trades_per_day=20, min_stop_ticks=1,
    )
    bis_fires = sum(
        1 for s in snaps
        if bis.evaluate_snapshot(s, bis_cfg, bis_state, atr=2.0,
                                   symbol="MNQ") is not None
    )
    assert bis_fires > 0
    ...
    mp_fires = sum(
        1 for s in snaps
        if mp.evaluate_snapshot(s, mp_cfg, mp_state, atr=2.0,
                                  symbol="MNQ") is not None
    )
    assert mp_fires > 0
    ...
    # aggressor_flow
    ag_state = ag.AggressorFlowState()
    ag_cfg = ag.AggressorFlowConfig(window_bars=5, consecutive_bars=1,
                                       entry_threshold=0.2,
                                       require_close_confirm=False,
                                       max_trades_per_day=20)
    ag_fires = sum(
        1 for b in bars
        if ag.evaluate_bar(b, ag_cfg, ag_state, atr=2.0,
                            symbol="MNQ") is not None
    )
    assert ag_fires > 0
...
def test_walk_forward_spread_regime_filter_pause_persists() -> None:
    ...
    cfg = srf.SpreadRegimeConfig(pause_at_multiple=4.0,
                                    resume_at_multiple=2.0,
                                    lookback_minutes=5)
    state = srf.SpreadRegimeState()
    base = datetime(2026, 5, 12, 14, 0, 0, tzinfo=UTC)
    # Build median at 0.25
    for i in range(20):
        snap = {"spread": 0.25, "bids": [{"price": 100, "size": 10}],
                "asks": [{"price": 100.25, "size": 10}]}
        srf.update_spread_regime(snap, cfg, state,
                                    now=base + timedelta(seconds=i))
    # Blow spread to 4x = 1.0 → PAUSE
    snap = {"spread": 1.0, "bids": [{"price": 100, "size": 10}],
            "asks": [{"price": 101, "size": 10}]}
    out = srf.update_spread_regime(snap, cfg, state,
    ...
    snap = {"spread": 0.5, "bids": [{"price": 100, "size": 10}],
            "asks": [{"price": 100.5, "size": 10}]}
    out = srf.update_spread_regime(snap, cfg, state,
...
def test_vps_deploy_register_script_exists_and_parses() -> None:
    ...
    script_path = (Path(__file__).resolve().parents[1] / "deploy"
                   / "scripts" / "register_l2_cron_tasks.ps1")
    assert script_path.exists(), f"missing: {script_path}"
    content = script_path.read_text(encoding="utf-8")
    # Daily tasks
    for task in ("ETA-L2-BacktestDaily", "ETA-L2-PromotionEvaluator",
                  "ETA-L2-CalibrationDaily", "ETA-L2-RegistryAdapter",
                  "ETA-L2-DriftMonitor", "ETA-L2-RiskMetrics"):
        assert task in content, f"missing daily task: {task}"
    # Weekly tasks
    for task in ("ETA-L2-SweepWeekly", "ETA-L2-FillAuditWeekly",
                  "ETA-L2-SlipRetrainWeekly", "ETA-L2-FillLatencyWeekly",
...
def test_vps_deploy_helper_script_exists_and_parses() -> None:
    ...
    helper_path = (Path(__file__).resolve().parents[1] / "deploy"
                   / "scripts" / "deploy_l2_to_vps.ps1")
...
def test_combined_gates_blackout_and_fuse_both_block(tmp_path: Path) -> None:
    ...
    fuse_check = fuse.check_fuse("book_imbalance_v1", "MNQ",
                                    _path=fuse_path)
