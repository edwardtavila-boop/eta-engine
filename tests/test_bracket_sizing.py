...
def test_compute_atr_uses_correct_period_window() -> None:
    ...
    bars = [{"high": 100.1, "low": 99.9, "close": 100.0}] * 5 + [{"high": 110, "low": 90, "close": 100}] * 15
    atr = compute_atr(bars, period=14)
    assert atr is not None
...
def test_cap_qty_uses_contract_multiplier_for_notional() -> None:
    ...
    try:
        ...
        assert reason == "per_bot_capped", (
            f"YM 0.1 contract at $24,820 notional MUST be capped at $10k; got reason={reason} qty={qty}"
        )
...
def test_paper_floor_disabled_in_live_mode() -> None:
    ...
    try:
        ...
        assert qty == 0.0, f"live mode must not lift sub-1-lot to 1; got qty={qty} reason={reason}"
        assert reason == "per_bot_capped"
    finally:
...
def test_per_bot_budget_override_lifts_cap_for_high_notional_contracts() -> None:
    ...
    with patch(
        ...
        try:
            ...
            assert default_cap == 10000.0, f"asset-class default should apply when bot_id is None; got {default_cap}"
        finally:
            os.environ.pop("ETA_LIVE_FUTURES_BUDGET_PER_BOT_USD", None)
...
def test_compute_bracket_fx_uses_5_decimal_precision() -> None:
    ...
    bars = [{"high": 1.171, "low": 1.170, "close": 1.1708, "open": 1.171} for _ in range(20)]
    stop, target, src = compute_bracket(
        side="BUY",
        entry_price=1.1708,
        bars=bars,
        stop_mult_override=2.0,
        target_mult_override=2.5,
    )
    assert src == "atr"
    ...
    assert abs(stop - 1.1708) >= 0.001, (
        f"FX stop distance {abs(stop - 1.1708):.6f} too small — 5-decimal precision should preserve it"
    )
...
def test_compute_bracket_btc_uses_2_decimal_precision() -> None:
    ...
    bars = [{"high": 100100, "low": 99900, "close": 100000, "open": 100050} for _ in range(20)]
    stop, target, src = compute_bracket(
        side="BUY",
...
def test_compute_bracket_min_distance_falls_through_to_fixed_pct() -> None:
    ...
    bars = [{"high": 1.170001, "low": 1.169999, "close": 1.17, "open": 1.170001} for _ in range(20)]
    stop, target, src = compute_bracket(
        side="BUY",
        entry_price=1.17,
        bars=bars,
