...
RUNNER = ROOT / "deploy" / "scripts" / "run_operator_queue_heartbeat_task.cmd"
REGISTRAR = ROOT / "deploy" / "scripts" / "register_operator_queue_heartbeat_task.ps1"
BOOTSTRAP = ROOT / "deploy" / "vps_bootstrap.ps1"
RUNBOOK = ROOT / "docs" / "VPS_OPS_HARDENING_RUNBOOK.md"


def test_operator_queue_runner_refreshes_canonical_snapshot() -> None:
    ...
    assert "-m eta_engine.scripts.operator_queue_heartbeat" in text
    assert '--out "%ETA_STATE_DIR%\\operator_queue_snapshot.json"' in text
    assert '--previous "%ETA_STATE_DIR%\\operator_queue_snapshot.previous.json"' in text
    assert "--cached-readiness" in text
    assert "--changed-only" in text
    assert "operator_queue_heartbeat.task.log" in text
    assert "exit /b %HEARTBEAT_RC%" in text


def test_operator_queue_registrar_is_canonical_read_only_and_low_overhead() -> None:
    text = REGISTRAR.read_text(encoding="utf-8")

    assert "ETA-OperatorQueueHeartbeat" in text
    assert '$WorkspaceRoot = "C:\\EvolutionaryTradingAlgo"' in text
    assert "Assert-CanonicalEtaPath" in text
    assert "run_operator_queue_heartbeat_task.cmd" in text
    assert "operator_queue_snapshot.json" in text
    assert "operator_queue_snapshot.previous.json" in text
...
def test_operator_queue_heartbeat_is_wired_into_bootstrap_and_runbook() -> None:
    bootstrap = BOOTSTRAP.read_text(encoding="utf-8")
    runbook = RUNBOOK.read_text(encoding="utf-8")

    assert "register_operator_queue_heartbeat_task.ps1" in bootstrap
    assert "ETA-OperatorQueueHeartbeat" in bootstrap
    assert "every 5m read-only queue snapshot" in bootstrap
    assert "register_operator_queue_heartbeat_task.ps1 -Start" in runbook
    assert "operator_queue_snapshot.json" in runbook
