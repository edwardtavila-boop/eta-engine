...
def _trade(bot_id: str, r: float, days_ago: float = 0, idx: int = 0, base_dt: datetime | None = None) -> dict:
...
def test_D2_r_bleed_demotes_decayed_strategy() -> None:
    ...
    trades = [_trade("bleeder", r=-0.3, days_ago=i * 0.1, idx=i) for i in range(50)]
...
def test_W2_r_drift_warns_when_recent_avg_r_eroded() -> None:
    ...
    trades = [_trade("drift", r=0.02, days_ago=i * 0.1, idx=i) for i in range(50)]
...
def test_KEEP_when_all_criteria_pass() -> None:
    ...
    trades = [_trade("healthy", r=0.5 if i % 2 == 0 else -0.2, days_ago=i * 0.2, idx=i) for i in range(60)]
...
def test_hard_failure_overrides_soft() -> None:
    ...
    trades = [_trade("doubly_bad", r=-0.5, days_ago=20 + i, idx=i) for i in range(15)]
...
def test_run_writes_json_receipt(tmp_path: Path, monkeypatch: object) -> None:
    ...
    can_path.write_text(
        "\n".join(
            json.dumps(r)
            for r in [
                _trade("m2k_sweep_reclaim", r=0.5 if i % 2 == 0 else -0.2, days_ago=i * 0.1, idx=i) for i in range(60)
            ]
        ),
        encoding="utf-8",
    )
