...
def _snap(
    bid_qtys: list[int], ask_qtys: list[int], *, mid: float = 100.0, spread: float = 0.25, ts: datetime | None = None
) -> dict:
    ...
    return {
        ...
        "bids": [{"price": mid - (i + 1) * 0.25, "size": s} for i, s in enumerate(bid_qtys)],
        "asks": [{"price": mid + (i + 1) * 0.25, "size": s} for i, s in enumerate(ask_qtys)],
        "spread": spread,
        "mid": mid,
    }
...
def test_B3_cooldown_bars_blocks_immediate_reentry() -> None:
    ...
    s2 = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=5)), cfg, state, atr=2.0)
    assert s2 is None
    # Snap 3 (10s later) — still within cooldown
    s3 = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=10)), cfg, state, atr=2.0)
    assert s3 is None
...
def test_B3_cooldown_bars_releases_after_window() -> None:
    ...
    s_post = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=20)), cfg, state, atr=2.0)
    assert s_post is None  # only 1 snap since cooldown released, need 2 consecutive
    s_fire = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=25)), cfg, state, atr=2.0)
...
def test_B3_cooldown_seconds_overrides_cooldown_bars() -> None:
    ...
    s2 = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=30)), cfg, state, atr=2.0)
    assert s2 is None
...
def test_B6_signal_id_includes_symbol_side_and_ts() -> None:
    ...
    state = bis.BookImbalanceState()
    base = datetime(2026, 5, 11, 14, 30, 0, tzinfo=UTC)
    sig = bis.evaluate_snapshot(_snap([50], [10], ts=base), cfg, state, atr=2.0, symbol="MNQ")
...
def test_B6_qty_contracts_hard_capped() -> None:
    cfg = bis.BookImbalanceConfig(entry_threshold=1.5, consecutive_snaps=1, max_qty_contracts=1)
    state = bis.BookImbalanceState()
    ...
    cfg2 = bis.BookImbalanceConfig(entry_threshold=1.5, consecutive_snaps=1, max_qty_contracts=999)
...
def test_B6_signal_carries_symbol() -> None:
    cfg = bis.BookImbalanceConfig(entry_threshold=1.5, consecutive_snaps=1)
    state = bis.BookImbalanceState()
    sig = bis.evaluate_snapshot(_snap([50], [10]), cfg, state, atr=2.0, symbol="ES")
    assert sig is not None
...
def test_I5_spread_filter_uses_correct_tick_for_GC() -> None:
    ...
    cfg = bis.BookImbalanceConfig(entry_threshold=1.5, consecutive_snaps=1, spread_min_ticks=1.0, spread_max_ticks=3.0)
    state = bis.BookImbalanceState()
    # spread=0.05 < 0.10 (1 tick) → rejected
    sig = bis.evaluate_snapshot(_snap([50], [10], spread=0.05), cfg, state, atr=2.0, symbol="GC")
    assert sig is None
    # spread=0.20 (2 ticks for GC) → accepted
    sig = bis.evaluate_snapshot(_snap([50], [10], spread=0.20), cfg, state, atr=2.0, symbol="GC")
    assert sig is not None
...
def test_I7_gap_too_large_resets_consecutive_count() -> None:
    ...
    sig = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=30)), cfg, state, atr=2.0)
    ...
    sig = bis.evaluate_snapshot(_snap([50], [10], ts=base + timedelta(seconds=35)), cfg, state, atr=2.0)
    assert sig is not None
...
def test_I6_long_pause_sets_warning_flag() -> None:
    cfg = srf.SpreadRegimeConfig(pause_at_multiple=4.0, resume_at_multiple=2.0, max_pause_seconds=10.0)
    state = srf.SpreadRegimeState()
    base = datetime(2026, 5, 11, 14, 30, 0, tzinfo=UTC)
    # Build median, then trigger pause
    for i in range(10):
        srf.update_spread_regime(_snap([10], [10], spread=0.25), cfg, state, now=base + timedelta(seconds=i))
    srf.update_spread_regime(_snap([10], [10], spread=2.0), cfg, state, now=base + timedelta(seconds=15))
    # 30s into pause → exceeds 10s max → warning flag fires
    out = srf.update_spread_regime(_snap([10], [10], spread=2.0), cfg, state, now=base + timedelta(seconds=45))
...
def test_B2_no_data_pre_captures_passes_open(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    dt = datetime.now(UTC)
    r = l2_overlay.confirm_sweep_with_l2(symbol="MNQ", swept_level=100.0, touch_dt=dt, side="LONG")
    assert r.passed is True
    assert r.reason == "no_l2_yet"


def test_B2_no_data_post_captures_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_B2_clear_captures_expected_per_symbol(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(l2_overlay, "DEPTH_DIR", tmp_path)
    l2_overlay.clear_captures_expected()
    dt = datetime.now(UTC)
    l2_overlay.mark_captures_expected("MNQ", when=dt)
    ...
    r_mnq = l2_overlay.confirm_poc_pull_with_l2(symbol="MNQ", entry_dt=dt, entry_side="LONG")
    r_es = l2_overlay.confirm_poc_pull_with_l2(symbol="ES", entry_dt=dt, entry_side="LONG")
...
def test_I2_sweep_takes_max_size_in_window(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    r = l2_overlay.confirm_sweep_with_l2(
        symbol="MNQ", swept_level=100.0, touch_dt=dt, side="LONG", min_stop_qty=50, window_seconds=60
    )
    assert r.passed is True
    assert r.detail["max_visible_qty"] == 60


def test_I2_hidden_qty_floor_rescues_thin_visible(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    r = l2_overlay.confirm_sweep_with_l2(
        symbol="MNQ", swept_level=100.0, touch_dt=dt, side="LONG", min_stop_qty=50, hidden_qty_floor=40
    )
    assert r.passed is True
    assert r.detail["effective_qty"] == 60


def test_I2_poc_pull_stale_snapshot_rejected(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    r = l2_overlay.confirm_poc_pull_with_l2(
        symbol="MNQ", entry_dt=dt, entry_side="LONG", min_imbalance_ratio=2.0, max_snapshot_staleness_seconds=5.0
    )
...
def test_I1_stop_fills_one_tick_worse_for_LONG() -> None:
    ...
    trade = l2_backtest_harness._simulate_exit_pessimistic(sig, future, point_value=2.0, tick_size=0.25)
...
def test_I1_commission_subtracted_from_net() -> None:
    ...
    assert (trade.pnl_dollars - trade.pnl_dollars_net) == pytest.approx(l2_backtest_harness.COMMISSION_PER_RT_USD)
...
def test_I4_backtest_skips_signals_during_regime_pause(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_I4_disable_regime_filter_via_flag(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_I9_walk_forward_split_present_with_enough_snaps(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(l2_backtest_harness, "DEPTH_DIR", tmp_path)
    today = datetime.now(UTC).replace(microsecond=0, second=0)
    base_epoch = today.timestamp()
    snaps = []
...
def test_I9_walk_forward_skipped_with_few_snaps(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_B5_no_disk_digest_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_B5_critical_disk_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(trading_gate, "DISK_LOG", tmp_path / "disk.jsonl")
    monkeypatch.setattr(trading_gate, "CAPTURE_HEALTH_LOG", tmp_path / "cap.jsonl")
    monkeypatch.setattr(trading_gate, "GATE_LOG", tmp_path / "gate.jsonl")
    trading_gate._reset_cache_for_tests()
    now_iso = datetime.now(UTC).isoformat()
    _write_jsonl(tmp_path / "disk.jsonl", {"ts": now_iso, "verdict": "CRITICAL", "worst_partition": "C:\\"})
...
def test_B5_red_capture_health_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(trading_gate, "DISK_LOG", tmp_path / "disk.jsonl")
    monkeypatch.setattr(trading_gate, "CAPTURE_HEALTH_LOG", tmp_path / "cap.jsonl")
    monkeypatch.setattr(trading_gate, "GATE_LOG", tmp_path / "gate.jsonl")
    trading_gate._reset_cache_for_tests()
    ...
    _write_jsonl(tmp_path / "cap.jsonl", {"ts": now_iso, "verdict": "RED", "issues": ["MNQ ticks STALE"]})
    decision = trading_gate.check_pre_trade_gate("MNQ", force_refresh=True)
    assert decision.blocked is True
    assert decision.reason == "capture_RED"


def test_B5_stale_disk_digest_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(trading_gate, "DISK_LOG", tmp_path / "disk.jsonl")
    monkeypatch.setattr(trading_gate, "CAPTURE_HEALTH_LOG", tmp_path / "cap.jsonl")
    monkeypatch.setattr(trading_gate, "GATE_LOG", tmp_path / "gate.jsonl")
    trading_gate._reset_cache_for_tests()
...
def test_B5_all_green_passes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(trading_gate, "DISK_LOG", tmp_path / "disk.jsonl")
    monkeypatch.setattr(trading_gate, "CAPTURE_HEALTH_LOG", tmp_path / "cap.jsonl")
    monkeypatch.setattr(trading_gate, "GATE_LOG", tmp_path / "gate.jsonl")
    trading_gate._reset_cache_for_tests()
...
def test_D2_rotation_digest_includes_pending_counters(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    digest = capture_rotation._process_kind(tmp_path / "ticks", "ticks", today, keep_days=14, cold_days=90, apply=False)
...
def test_D3_dashboard_marks_dryrun_with_pending_as_yellow(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setitem(health_dashboard.SOURCES, "capture_rotation", tmp_path / "rot.jsonl")
    rot_record = {
        "ts": datetime.now(UTC).isoformat(),
        "apply": False,
        "ticks": {"n_compressed": 0, "n_cold_archived": 0, "n_would_compress": 5, "n_would_cold_archived": 0},
        "depth": {"n_compressed": 0, "n_cold_archived": 0, "n_would_compress": 3, "n_would_cold_archived": 1},
...
def test_D3_dashboard_dryrun_with_no_pending_stays_dryrun(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setitem(health_dashboard.SOURCES, "capture_rotation", tmp_path / "rot.jsonl")
    rot_record = {
        "ts": datetime.now(UTC).isoformat(),
        "apply": False,
        "ticks": {"n_compressed": 0, "n_cold_archived": 0, "n_would_compress": 0, "n_would_cold_archived": 0},
        "depth": {"n_compressed": 0, "n_cold_archived": 0, "n_would_compress": 0, "n_would_cold_archived": 0},
        "totals": {"n_compressed": 0, "n_cold_archived": 0},
    }
    (tmp_path / "rot.jsonl").write_text(json.dumps(rot_record) + "\n", encoding="utf-8")
    d = health_dashboard.build_dashboard(alert_hours=24)
...
def test_D6_disk_monitor_alert_failure_writes_to_stderr(
