...
from apex_predator.brain.avengers.circuit_breaker import BreakerState, BreakerTripped
from apex_predator.brain.avengers.watchdog import HealthStatus
from apex_predator.brain.jarvis_admin import SubsystemId
from apex_predator.brain.model_policy import ModelTier
from apex_predator.scripts import chaos_drill
...
class TestSharedCircuitBreaker:
    ...
    def test_default_path_is_home_jarvis(self) -> None:
        ...
        from apex_predator.brain.avengers import DEFAULT_BREAKER_PATH
...
class TestChaosDrill:
    ...
    def test_format_report_contains_pass_markers(self, tmp_path: Path) -> None:
        ...
        assert "APEX PREDATOR // CHAOS DRILL" in report
...
class TestChaosDrillCronHandler:
    ...
    def test_enum_exists_and_owned_by_alfred(self) -> None:
        from apex_predator.brain.avengers import TASK_OWNERS
    ...
    def test_cadence_is_monthly_first_at_3am(self) -> None:
        from apex_predator.brain.avengers.dispatch import TASK_CADENCE
    ...
    def test_crontab_contains_chaos_drill_entry(self) -> None:
        ...
        import apex_predator
        # Package root -> parent is apex_predator/
        repo = Path(apex_predator.__file__).resolve().parent
    ...
    def test_handler_writes_report_and_history(self, tmp_path: Path) -> None:
        ...
        from apex_predator.deploy.scripts.run_task import _task_chaos_drill
...
class TestDashboardDriftPanel:
    ...
    def dashboard_module(self, tmp_path: Path, monkeypatch):
        ...
        import apex_predator.scripts.jarvis_dashboard as mod
...
class TestPushBusDedup:
    ...
    def test_dedup_suppresses_repeat_titles(self) -> None:
        from apex_predator.brain.avengers import AlertLevel, PushBus
...
def capture_bus():
    ...
    NB: ``apex_predator.brain.avengers.push`` resolves to the re-exported
    ...
    from apex_predator.brain.avengers import PushBus
    push_mod = importlib.import_module("apex_predator.brain.avengers.push")
...
class TestRunTaskAlertWiring:
    ...
    def test_main_pushes_on_handler_exception(
        ...
        capture = capture_bus
        from apex_predator.deploy.scripts import run_task
    ...
    def test_chaos_drill_pushes_on_failure(
        ...
        monkeypatch.setattr(
            "apex_predator.scripts.chaos_drill.run_drills",
            lambda: fake_results,
        )
        out = _task_chaos_drill(tmp_path)
        assert out["failed"] == 2
