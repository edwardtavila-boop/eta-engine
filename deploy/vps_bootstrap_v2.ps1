# VPS Bootstrap v2 — one-command full stack startup
# Wave-19 (2026-05-04): Adds WinSW service registration, ETA persona tasks,
# Force Multiplier CLI checks, dual venv validation, and IBKR path fix.
#
...
Write-Host "  ETA VPS Bootstrap v2 — Full Stack Registration" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan
...
if (-not $SkipForceMultiplier) {
    ...
    $claudeCheck = (Get-Command claude -ErrorAction SilentlyContinue) -or (Get-Command npx -ErrorAction SilentlyContinue)
    if ($claudeCheck) {
...
Write-Host "  FirmCommandCenter                — dashboard (127.0.0.1:8420)" -ForegroundColor Gray
