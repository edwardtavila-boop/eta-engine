...
foreach ($entry in $PSBoundParameters.GetEnumerator()) {
    if ($entry.Key -eq "SkipKaizen") {
        continue
    }
    $forwardParams[$entry.Key] = $entry.Value
}

Write-Host "Compatibility bootstrap notice: forwarding vps_bootstrap_v1_legacy.ps1 to $canonicalBootstrap" -ForegroundColor Yellow
if ($SkipKaizen) {
    Write-Host "Legacy flag SkipKaizen is deprecated and will be ignored by the canonical bootstrap." -ForegroundColor Yellow
}

& $canonicalBootstrap @forwardParams
