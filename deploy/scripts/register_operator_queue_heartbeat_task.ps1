# EVOLUTIONARY TRADING ALGO // register_operator_queue_heartbeat_task.ps1
# Registers a low-overhead read-only operator-queue snapshot heartbeat.
...
$TaskName = "ETA-OperatorQueueHeartbeat"
...
$Runner = Join-Path $WorkingDir "deploy\scripts\run_operator_queue_heartbeat_task.cmd"
$Artifact = Join-Path $StateDir "operator_queue_snapshot.json"
$PreviousArtifact = Join-Path $StateDir "operator_queue_snapshot.previous.json"
...
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing operator queue heartbeat runner: $Runner"
}
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Refreshes the read-only ETA operator queue snapshot under C:\EvolutionaryTradingAlgo." `
