# EVOLUTIONARY TRADING ALGO // register_broker_state_refresh_task.ps1
# Registers a low-overhead read-only broker-state cache refresh heartbeat.
...
$TaskName = "ETA-BrokerStateRefreshHeartbeat"
...
$Runner = Join-Path $WorkingDir "deploy\scripts\run_broker_state_refresh_task.cmd"
$Artifact = Join-Path $StateDir "broker_state_refresh_heartbeat.json"
...
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing broker-state refresh runner: $Runner"
}
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Refreshes the read-only ETA broker-state cache under C:\EvolutionaryTradingAlgo." `
