...
set "SCRIPT_NAME=repair_eta_public_edge_route_watchdog_admin.cmd"
set "REPAIR_PS1=%SCRIPTS%\repair_eta_public_edge_route_watchdog_task.ps1"
set "DRY_RUN=0"
set "NO_ELEVATE=0"
...
if not exist "%REPAIR_PS1%" (
    echo Missing public edge watchdog repair script:
    echo   %REPAIR_PS1%
    exit /b 3
)
...
if errorlevel 1 (
    if "%NO_ELEVATE%"=="1" (
        echo Administrator rights are required to repair ETA-Public-Edge-Route-Watchdog.
    ...
    echo Requesting Administrator approval to repair ETA-Public-Edge-Route-Watchdog.
...
echo.
echo   /DryRun     Validate the canonical ETA-Public-Edge-Route-Watchdog repair without registering the task.
