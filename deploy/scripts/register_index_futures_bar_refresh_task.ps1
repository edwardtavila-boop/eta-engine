# EVOLUTIONARY TRADING ALGO // register_index_futures_bar_refresh_task.ps1
# Registers the lightweight NQ/MNQ replay bar refresh heartbeat.
...
$TaskName = "ETA-IndexFutures-Bar-Refresh"
...
$Runner = Join-Path $WorkingDir "deploy\scripts\run_index_futures_bar_refresh_task.cmd"
$Artifact = Join-Path $StateDir "index_futures_bar_refresh_latest.json"
...
if ($IntervalMinutes -lt 5 -or $IntervalMinutes -gt 60) {
    throw "IntervalMinutes must stay between 5 and 60 for futures-bar freshness."
}
...
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing index futures bar refresh runner: $Runner"
}
...
if ($DryRun) {
    ...
    Write-Host "  Data path   : refreshes NQ/MNQ canonical 5-minute futures history via public yfinance fallback"
    Write-Host "  Order action: never submits, cancels, flattens, acknowledges, or promotes"
    exit 0
}
...
$description = "ETA index futures bar refresh: keeps NQ/MNQ 5-minute replay bars fresh under C:\EvolutionaryTradingAlgo using public yfinance fallback; data-only, no order actions."
