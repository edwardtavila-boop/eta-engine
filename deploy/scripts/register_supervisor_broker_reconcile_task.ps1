# EVOLUTIONARY TRADING ALGO // register_supervisor_broker_reconcile_task.ps1
# Registers a low-overhead read-only broker-vs-supervisor reconcile heartbeat.
...
$TaskName = "ETA-SupervisorBrokerReconcile"
...
$Runner = Join-Path $WorkingDir "deploy\scripts\run_supervisor_broker_reconcile_task.cmd"
$Artifact = Join-Path $StateDir "jarvis_intel\supervisor\reconcile_last.json"
$StatusArtifact = Join-Path $StateDir "supervisor_broker_reconcile_heartbeat.json"
...
Assert-CanonicalEtaPath -Path $StatusArtifact

if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing supervisor-broker reconcile runner: $Runner"
}
...
if ($DryRun) {
    ...
    Write-Host "  Status      : $StatusArtifact"
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Refreshes the read-only ETA broker-vs-supervisor reconcile artifact under C:\EvolutionaryTradingAlgo." `
