# EVOLUTIONARY TRADING ALGO // register_paper_live_transition_check_task.ps1
# Registers a low-overhead cache refresher for the paper-live transition gate.
...
$TaskName = "ETA-PaperLiveTransitionCheck"
...
$Runner = Join-Path $WorkingDir "deploy\scripts\run_paper_live_transition_check.cmd"
$Artifact = Join-Path $StateDir "paper_live_transition_check.json"
...
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Missing paper-live transition task runner: $Runner"
}
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Refreshes the read-only ETA paper-live transition gate cache under C:\EvolutionaryTradingAlgo." `
