...
if (-not $SkipETATasks) {
    ...
    $publicEdgeRouteWatchdogTaskScript = "$EtaEngineDir\deploy\scripts\register_public_edge_route_watchdog_task.ps1"
    ...
    if (Test-Path $publicEdgeRouteWatchdogTaskScript) {
        Write-Host "  Registering public edge route watchdog task (keeps 127.0.0.1:8081 aligned to 8421)..." -ForegroundColor Gray
        if (-not $WhatIf) {
            & $pwshPath -ExecutionPolicy Bypass -File $publicEdgeRouteWatchdogTaskScript -Start -RestartExistingProcess
        }
    } else {
        Write-Host "  register_public_edge_route_watchdog_task.ps1 not found at $publicEdgeRouteWatchdogTaskScript" -ForegroundColor Yellow
    }
...
Write-Host "  ETA-Public-Edge-Route-Watchdog   -- boot/logon + every 5m 8081 route self-heal" -ForegroundColor Gray
