...
class RangeStrategy:
    ...
    def _bb(self) -> tuple[float, float, float]:
        if len(self._close_window) < self.cfg.bb_period:
            return (0, 0, 0)
    ...
    def _detect_reversion(self, bar: BarData) -> str | None:
        ...
        if len(volumes) < self.cfg.volume_z_lookback:
            return None
        mean_v = sum(volumes) / len(volumes)
        std_v = (sum((v - mean_v) ** 2 for v in volumes) / len(volumes)) ** 0.5
        if std_v > 0 and (bar.volume - mean_v) / std_v < self.cfg.min_volume_z:
            return None
