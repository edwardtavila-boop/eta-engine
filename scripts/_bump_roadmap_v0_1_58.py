...
----------------------
v0.1.57 wired D1..D5 into ``scripts/run_apex_live.py`` and shipped
...
while the Apex eval was either already busted or about to bust:
  ...
  B1 -- ``run_apex_live.py`` keyed today's consistency-guard entry by
...
B1 fix -- Apex session-day helper
  * ``apex_predator/core/consistency_guard.py`` -- new
  ...
  * ``scripts/run_apex_live.py`` -- replaced the ``utc_today_iso``
    callsite with ``apex_trading_day_iso()``. Import updated.

B2 fix -- live-mode gate on tracker presence
  * ``scripts/run_apex_live.py`` -- ``ApexRuntime.__init__`` now
...
--------------
  ...
  * ``tests/test_run_apex_live.py`` +4 tests
    ...
    (dry_run wins).
  * ``tests/test_run_apex_live.py`` +2 tests
...
Pre-existing ANN401 debt in ``run_apex_live.py`` remains as
...
----------------------
  ...
  * ``apex_predator_tests_passing`` incremented by 17 new tests.
  * New key ``apex_predator_v0_1_58_red_team_closure`` with a full
...
def main() -> None:
    ...
    sa["apex_predator_v0_1_58_red_team_closure"] = {
        "timestamp_utc": now,
        "version": VERSION,
        "bundle_name": (
        ...
        "blockers_closed": {
            "B1_session_day_bucketing": {
                ...
                "fix": (
                    ...
                    "run_apex_live now calls this helper."
                ),
        ...
        "pre_existing_lint_debt": (
            "ANN401/ANN002/ANN003 in run_apex_live.py pre-date "
