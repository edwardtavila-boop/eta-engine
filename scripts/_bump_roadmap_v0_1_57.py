...
into ``scripts/run_apex_live.py`` end-to-end, and runs the bootstrap
...
D1 ``session_gate`` wiring
  * ``apex_predator/bots/mnq/bot.py`` attaches ``SessionGate`` at
...
D2 ``trailing_dd_tracker`` module + wiring
  * ``apex_predator/core/trailing_dd_tracker.py`` -- new module.
  ...
  * ``scripts/run_apex_live.py`` now accepts a
...
D3 ``consistency_guard`` module + wiring
  * ``apex_predator/core/consistency_guard.py`` -- new module.
...
D5 ``kill_switch_latch`` wiring
  * ``KillSwitchLatch.boot_allowed()`` guards startup in
    ``run_apex_live`` -- a tripped latch blocks the loop from
    initializing.
...
Tests
  ...
  * ``tests/test_run_apex_live.py`` -- 4 new async tests for D2
  ...
  * Ruff-clean on every new file. Pre-existing ANN401/SIM105/
    TC003-Callable debt in ``run_apex_live.py`` is unrelated to
...
-----
  * tests_passing: 3251 -> 3499 (+248)
    ...
    and the integration suite in ``test_run_apex_live.py``)
...
def main() -> None:
    ...
    sa["apex_predator_v0_1_57_apex_eval_readiness"] = {
        "timestamp_utc": now,
        "version": VERSION,
        "bundle_name": (
            "APEX EVAL READINESS CLOSURE -- D1..D5 wired end-to-end "
            "in run_apex_live + MnqBot; bootstrap CI on real-MNQ OOS "
        ...
        "modules_modified": [
            ...
            "apex_predator/bots/mnq/bot.py "
            "(SessionGate attach + on_bar EoD flatten path)",
            "apex_predator/venues/router_adapter.py "
            "(session gate surface + should_flatten_eod signal)",
        ],
        "tests_added": [
            "apex_predator/tests/test_trailing_dd_tracker.py (30)",
            "apex_predator/tests/test_consistency_guard.py (32)",
        ],
        "tests_extended": [
            ...
            "apex_predator/tests/test_session_gate.py",
            "apex_predator/tests/test_mnq_bot.py (EoD flatten path)",
            "apex_predator/tests/test_router_adapter.py (session gate "
            "+ should_flatten_eod)",
        ],
        "d_series_wiring": {
            "D1_session_gate": (
            ...
            "D5_kill_switch_latch": (
                "run_apex_live calls KillSwitchLatch.boot_allowed() "
        ...
        "bootstrap_ci_mnq_oos": {
            "script": "apex_predator/scripts/bootstrap_ci_mnq_oos.py",
            "iterations": 10000,
            "seed": 11,
            "n_bars_total": 5642,
            "n_bars_oos": 1693,
            ...
            "verdict": "NOISE",
            "artifact_json": (
                "apex_predator/docs/cross_regime/"
        ...
        "design_choices": {
            ...
            "type_checking_hygiene": (
                ...
                "in run_apex_live because it only appears in "
        ...
        "pre_existing_lint_debt_in_run_apex_live": (
            "ANN401/ANN002/ANN003/SIM105/TC003-Callable errors in "
            "run_apex_live.py pre-date this bump and are tracked "
        ...
        milestones.append(
            {
                ...
                "title": (
                    ...
                    "wired end-to-end in run_apex_live and covered "
