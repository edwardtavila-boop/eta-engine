...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(records) < args.min_decisions:
        logger.info("not enough records (%d < %d) -- skipping promotion check", len(records), args.min_decisions)
        return 0

    champion = champion_metrics(records)

    candidates_to_check = [c["name"] for c in list_candidates() if c["name"] != "v17"]
    ...
    for name in candidates_to_check:
        ...
        approval_diff = abs(float(metrics.get("approval_rate", 0)) - float(champion.get("approval_rate", 0)))
        ...
        logger.info(
            "  %s: differentiated=%s within_delta=%s enough_decisions=%s -> %s",
            name,
            differentiated,
            within_delta,
            enough_decisions,
