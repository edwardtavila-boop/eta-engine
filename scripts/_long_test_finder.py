...
------
    test-durations: 2255 tests, 8.2s total, top 10:
       ...
       0.21s  setup   tests/test_apex_overview.py::test_compact
       ...
...
def _run(top: int) -> tuple[int, str]:
    cmd = [
        ...
        "--tb=no", "--disable-warnings", f"--durations={top}",
    ]
    try:
        proc = subprocess.run(
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--max-yellow-ms", type=int, default=200,
                   help="single-test duration > N ms triggers YELLOW")
    p.add_argument("--max-red-count", type=int, default=5,
                   help="more than N over threshold -> RED")
    ...
    if not durations:
        print("long-test-finder: GREEN -- no per-test durations parsed "
    ...
    if args.json:
        print(json.dumps({
            ...
            "top": [
                {"dur_s": d, "phase": ph, "test": t} for d, ph, t in durations
            ],
        }, indent=2))
        return (
            0 if not over
            else 2 if len(over) > args.max_red_count
            else 1
        )
    ...
    print(
        f"long-test-finder: {level} -- {len(over)} test(s) over "
        f"{args.max_yellow_ms}ms (parsed {len(durations)})",
    )
    print()
