...
def _pair_by_time(snaps_a: list[dict], snaps_b: list[dict],
                   *, max_skew_seconds: float = 2.0) -> list[tuple[dict, dict]]:
    ...
    for a in snaps_a_sorted:
        ...
        while j + 1 < len(snaps_b_sorted) and \
                abs(snaps_b_sorted[j + 1].get("epoch_s", 0) - ea) < \
                abs(snaps_b_sorted[j].get("epoch_s", 0) - ea):
            j += 1
...
def check_discrepancy(snaps_a: list[dict], snaps_b: list[dict],
                       *, threshold_bps: float = 5.0,
                       max_skew_seconds: float = 2.0,
                       max_events: int = 100) -> CrossBrokerReport:
    ...
    if not pairs:
        return CrossBrokerReport(
            ...
            n_pairs_checked=0, n_discrepancies=0,
            discrepancy_pct=0.0, threshold_bps=threshold_bps,
            mean_diff_bps=None, p90_diff_bps=None, max_diff_bps=None,
    ...
    return CrossBrokerReport(
        ...
        mean_diff_bps=round(statistics.mean(all_diff_bps), 3)
                          if all_diff_bps else None,
...
def main() -> int:
    ...
    ap.add_argument("--symbol-b", default="NQ",
                    help="Second symbol (or different broker for same symbol)")
    ...
    ap.add_argument("--path-a", type=Path, default=None,
                    help="Override depth file path for symbol_a")
    ...
    if report.events:
        ...
        for e in report.events[:5]:
            print(f"    t={e.epoch_s:.0f}  diff={e.diff_pts:+.4f}pts"
