...
def _read_trades_dual_source() -> list[dict[str, Any]]:
    ...
    which now classifies every record by data_source (live/paper/backtest/
    historical_unverified/test_fixture) and filters out the latter two by
    default. Without this filter the legacy archive injected ~43k backtest
    emissions that inflated composite scores ~17x for some bots (m2k 1151
    "trades" vs ~4 actual live).
    """
