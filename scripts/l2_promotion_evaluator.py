...
def _read_recent_jsonl(path: Path, *, since_days: int = 30,
...
def _evaluate_shadow_to_paper(strategy_id: str, symbol: str) -> list[CriterionResult]:
    ...
    n_clean_days = sum(1 for r in cap_records
    ...
    bt_records = _read_recent_jsonl(L2_BACKTEST_LOG, since_days=30,
                                      strategy=harness_strategy, symbol=symbol)
    ...
    risk_alerts = [a for a in alerts
                    if a.get("level") in ("RED", "CRITICAL")
                    and "risk" in str(a.get("source", "")).lower()]
    results.append(CriterionResult(
...
def _evaluate_paper_to_live(strategy_id: str, symbol: str) -> list[CriterionResult]:
    ...
    sweep_records = _read_recent_jsonl(L2_SWEEP_LOG, since_days=30,
                                          strategy=harness_strategy, symbol=symbol)
...
def _evaluate_retirement(strategy_id: str, symbol: str) -> list[CriterionResult]:
    ...
    results: list[CriterionResult] = []
    harness_strategy = _harness_strategy_name(strategy_id)
    bt_records = _read_recent_jsonl(L2_BACKTEST_LOG, since_days=60,
                                      strategy=harness_strategy, symbol=symbol)
    ...
    rolling_14d = _read_recent_jsonl(L2_BACKTEST_LOG, since_days=14,
                                         strategy=harness_strategy, symbol=symbol)
    worst_sharpe = None
...
def evaluate_all() -> list[StrategyEvaluation]:
    ...
    return [evaluate_strategy(s.bot_id) for s in L2_STRATEGIES
             if s.max_qty_contracts > 0]  # skip filters (e.g. spread_regime)


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--strategy", default=None,
                    help="Specific bot_id; default = all entry strategies")
