...
html_files = [f for f in ROOT.rglob("*.html") if "node_modules" not in str(f) and ".venv" not in str(f)]
...
proc = subprocess.Popen(
    [str(python_path), "-m", "uvicorn", "command_center.server.app:app", "--host", "127.0.0.1", "--port", "8000"],
    env=env,
