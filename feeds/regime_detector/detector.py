...
class CrossAssetRegimeDetector:
    ...
    def run(self) -> RegimeState:
        ...
        primary, confidence, scores = self._classify_primary(
            correlations, vol_regime, drawdown_regime, per_asset
        )
    ...
    def _per_asset_regime(
        self, bars: dict[str, np.ndarray], returns: dict[str, np.ndarray]
    ) -> dict[str, str]:
