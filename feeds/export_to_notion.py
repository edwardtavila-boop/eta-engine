...
def _build_digest(*, audit_dir: Path, kaizen_ledger: Path, critique_dir: Path, bandit_dir: Path) -> dict[str, Any]:
    ...
    if today_bandit.exists():
        try:
            ...
            digest["promotable_candidates"] = [pr["candidate"] for pr in data.get("promotable", [])]
        except (json.JSONDecodeError, OSError):
            pass
...
def _post_notion(token: str, database_id: str, digest: dict[str, Any]) -> bool:
    ...
    body = {
        ...
        "properties": {
            "Name": {"title": [{"text": {"content": f"ETA Engine — {digest['date']}"}}]},
            "Date": {"date": {"start": digest["date"]}},
            "Kaizen +1": {
                "rich_text": [
                    {"text": {"content": (digest.get("kaizen_plus_one") or {}).get("title", "(none)")[:1900]}}
                ]
            },
            "Critique Severity": {
                "rich_text": [{"text": {"content": str((digest.get("critique_severity") or {}).get("severity", "—"))}}]
            },
            "Promotable": {
                "rich_text": [{"text": {"content": ", ".join(digest.get("promotable_candidates", [])) or "—"}}]
            },
            "Verdicts Total": {"number": sum((digest.get("verdicts_today") or {}).get("totals", {}).values())},
        },
    }
...
def _post_airtable(token: str, base_id: str, table_name: str, digest: dict[str, Any]) -> bool:
    body = {
        "fields": {
            "Date": digest["date"],
