...
class DataQualityMonitor:
    ...
    def _check_bar_feed(self, symbol: str, now: datetime) -> dict[str, Any]:
        ...
        for path in candidates:
            if path.is_file():
                ...
                return {
                    "feed_name": "bar_csv", "symbol": symbol, "status": "healthy",
                    "last_timestamp": mtime.isoformat(), "age_seconds": round(age, 0),
                }
        return {
            "feed_name": "bar_csv", "symbol": symbol, "status": "missing",
            "last_timestamp": None, "age_seconds": None,
    ...
    def _check_ibkr_feed(self, now: datetime) -> dict[str, Any]:
        ...
        try:
            ...
            s.close()
            return {
                "feed_name": "ibkr_tws", "symbol": "MNQ", "status": "healthy",
                "last_timestamp": now.isoformat(), "age_seconds": 0,
            }
        except Exception:
            return {
                "feed_name": "ibkr_tws", "symbol": "MNQ", "status": "critical",
                "last_timestamp": None, "age_seconds": None,
        ...
        for dd in data_dirs:
            if dd.is_dir():
                ...
                if files:
                    ...
                    if age > _STALENESS_THRESHOLD_MINUTES * 60:
                        return {
                            "feed_name": f"{exchange}_crypto", "symbol": symbol,
                            "status": "stale", "last_timestamp": mtime.isoformat(),
                            "age_seconds": round(age, 0),
                        }
                    return {
                        "feed_name": f"{exchange}_crypto", "symbol": symbol,
                        "status": "healthy", "last_timestamp": mtime.isoformat(),
                        "age_seconds": round(age, 0),
                    }
        return {
            "feed_name": f"{exchange}_crypto", "symbol": symbol,
            "status": "missing", "last_timestamp": None, "age_seconds": None,
        }
